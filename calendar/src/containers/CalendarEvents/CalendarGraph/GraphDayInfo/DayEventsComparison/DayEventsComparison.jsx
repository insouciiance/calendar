...
const DayEventsComparison = props => {
    ...
    const ordinaryEventsDifference =
        (currentDay.ordinaryEventsCount - dayToCompare.ordinaryEventsCount) *
        diffCoeff;
    const compellingEventsDifference =
        (currentDay.compellingEventsCount -
            dayToCompare.compellingEventsCount) *
        diffCoeff;
    const importantEventsDifference =
        (currentDay.importantEventsCount - dayToCompare.importantEventsCount) *
        diffCoeff;

    let ordinaryArrow = <i className="fas fa-arrows-alt-h" />;
    if (ordinaryEventsDifference > 0) {
        ordinaryArrow = (
            <i className="fas fa-angle-up" style={{ color: 'lightgreen' }} />
        );
    } else if (ordinaryEventsDifference < 0) {
        ordinaryArrow = (
            <i className="fas fa-angle-down" style={{ color: '#dc143c ' }} />
        );
    }
    ...
    return (
        <div className={classes.DayEventsComparison}>
            ...
            <div
                ...
                <span style={{ color: ordinaryColor }}>
                    {dayToCompare.ordinaryEventsCount}
                </span>
                {ordinaryArrow}
            ...
            <div
                className={classes.EventsTypeComparison}
                style={{ flexDirection: timeDiff > 0 ? 'row' : 'row-reverse' }}>
                <span style={{ color: compellingColor }}>
                    {dayToCompare.compellingEventsCount}
                </span>
                {compellingArrow}
            ...
            <div
                className={classes.EventsTypeComparison}
                style={{ flexDirection: timeDiff > 0 ? 'row' : 'row-reverse' }}>
                <span style={{ color: importantColor }}>
                    {dayToCompare.importantEventsCount}
                </span>
                {importantArrow}
