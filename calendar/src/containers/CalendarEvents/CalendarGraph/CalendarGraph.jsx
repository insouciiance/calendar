...
class CalendarGraph extends Component {
	...
	componentDidMount() {
		...
		this.ctx.canvas.addEventListener('mouseleave', () => this.updateCanvas());
	}
	...
	componentDidUpdate() {
		...
		const maxEventsInDay = Math.max(
			...graphInfo.map(x => x.ordinaryEvents + x.compellingEvents + x.importantEvents)
		);
		...
		for (let i = 0; i < graphInfo.length; i++) {
			...
			const yPos =
				this.graphHeight -
				((dayInfo.ordinaryEvents + dayInfo.compellingEvents + dayInfo.importantEvents) /
					maxEventsInDay) *
					this.graphHeight *
					0.9;
	...
	mouseClickHandler = e => {
		...
		this.daysInfos.forEach((day, index) => {
			if (Math.abs(x - day.xPos) < this.xDiff / 2) {
				this.setState({
					selectedGraphDay: selectedDay.date !== day.date ? day : null,
					dayBeforeSelected: index > 0 ? this.daysInfos[index - 1] : null,
					dayAfterSelected:
						index < this.daysInfos.length - 1 ? this.daysInfos[index + 1] : null
				});
			}
		});
	};
	...
	updateCanvas() {
		...
		this.daysInfos.forEach(day => {
			...
			const eventsInDay =
				day.ordinaryEventsCount + day.compellingEventsCount + day.importantEventsCount;

			if (eventsInDay > 0) {
				const ordinaryEventsColorStop = day.ordinaryEventsCount ? 0 : null;
				...
				const importantEventsColorStop = day.importantEventsCount ? 1 : null;

				gradient = ctx.createLinearGradient(0, day.yPos, 0, height);
				if (ordinaryEventsColorStop !== null) {
					gradient.addColorStop(ordinaryEventsColorStop, ordinaryColor);
				}
				if (compellingEventsColorStop !== null) {
					gradient.addColorStop(compellingEventsColorStop, compellingColor);
				}
				if (importantEventsColorStop !== null) {
					gradient.addColorStop(importantEventsColorStop, importantColor);
				}
	...
	render() {
		const { selectedGraphDay, dayBeforeSelected, dayAfterSelected } = this.state;
		const { ordinaryColor, compellingColor, importantColor } = this.props.customization;
		...
		const backdrop =
			tutorialIndex === 3 ? (
				<TutorialBackdrop nextTutorialIndex={4}>
					<p>Right here you&apos;re able to see all the events in a simple graph</p>
					<p>The number of days in the graph depends on the width of the screen</p>
					<p>Every day is also clickable!</p>
				</TutorialBackdrop>
			) : null;

		return (
			<div className={classes.CalendarGraphWrapper} ref={this.CanvasGraphWrapperDOM}>
				<p className={classes.Forecast}>{this.daysCount}-day Forecast</p>
