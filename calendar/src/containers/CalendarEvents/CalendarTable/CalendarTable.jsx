...
class CalendarTable extends PureComponent {
    ...
    componentDidMount() {
        this.setState({
            wrapperWidth: document.querySelector(`.${classes.ColumnWrapper}`)
                .offsetWidth,
        });
        ...
        const resizedw = () => {
            this.setState({
                wrapperWidth: document.querySelector(
    ...
    render() {
        ...
        if (eventsList === []) {
            return (
                <div className={classes.ColumnWrapper} ref={this.wrapperDOM} />
            );
        }
            ...
            .forEach(x => {
                ...
                if (
                    allRelevantEvents[datesArray.indexOf(dateFormat)].filter(
                        checkingEvent =>
                            checkingEvent.importance === x.importance,
                    ).length === 0
                ) {
                ...
                <div className={classes.CalendarScheduleChartTableContentInner}>
                    {allRelevantEvents.map((dayEvents, index) => (
                        <div
                            ...
                            style={{
                                width: `${
                                    (100 - 2 * daysToShow) / daysToShow
                                }%`,
                            }}>
                            {dayEvents.map(x => {
                                const dateFormat = moment(x.date).format(
                                    'YYYY-MM-DD',
                                );
                                let fittingColor;
                                ...
                                return (
                                    <CalendarTableBar
                                        ...
                                        key={x.id}
                                        isSelected={
                                            x.id === this.state.selectedEventId
                                        }
                                        ...
                                        eventsCount={
                                            allRelevantEventsCount[
                                                datesArray.indexOf(dateFormat)
                                            ][x.importance]
                                        }
                                    />
                                );
